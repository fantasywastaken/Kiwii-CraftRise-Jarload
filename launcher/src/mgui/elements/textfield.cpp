...
bool input_text_ex(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect clickable(rect.Min, rect.Max - ImVec2(25, 0));
    ...
    gui->push_font(var->font.inter[3]);
    ...
    text_filed_state* animstate = gui->anim_container(&animstate, id);
    animstate->alpha = ImClamp(animstate->alpha + (gui->fixed_speed(8.f) * (strlen(buf_display) > 0 ? 1.f : -1.f)), 0.f, 1.f);
    animstate->text_offset = ImLerp(animstate->text_offset, strlen(buf_display) > 0 ? clickable.GetHeight() / 2 : 0, gui->fixed_speed(12.f));
    animstate->line_offset = ImLerp(animstate->line_offset, render_cursor ? rect.GetWidth() / 2 : 0, gui->fixed_speed(12.f));

    // Render frame
    if (!is_multiline)
    {
        draw->text_clipped(window->DrawList, var->font.icons[2], ImVec2(clickable.Max.x, rect.Min.y), rect.Max, draw->get_clr(clr->textfield.text), hint, NULL, NULL, ImVec2(1.f, 0.5f));
        draw->rect_filled(window->DrawList, ImVec2(rect.Min.x, rect.Max.y - 1), rect.Max, draw->get_clr(clr->textfield.stroke));
        draw->rect_filled(window->DrawList, ImVec2(rect.GetCenter().x - animstate->line_offset, rect.Max.y - 1), ImVec2(rect.GetCenter().x + animstate->line_offset, rect.Max.y), draw->get_clr(clr->accent));

        window->DrawList->PushClipRect(rect.Min, rect.Max - ImVec2(0, 1), true);
        draw->text_clipped(window->DrawList, g.Font, rect.Min + ImVec2(0, animstate->text_offset), ImVec2(clickable.Max.x, rect.Max.y + animstate->text_offset), draw->get_clr(clr->textfield.text_inactive, 1.f - animstate->alpha), label, NULL, NULL, ImVec2(0.f, 0.5f));
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : ImVec2(clickable.Min.x, clickable.GetCenter().y - (flags & ImGuiInputTextFlags_Password ? 7 : 9));
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect d(ImVec2(rect_pos.x + 0.0f, clickable.Min.y + 12), ImVec2(rect_pos.x + rect_size.x, clickable.Max.y - 11));
                    d.ClipWith(clip_rect);
                    if (d.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(d.Min, d.Max, draw->get_clr(clr->accent, 0.3f));
                }
        ...
        if (render_cursor)
        {
            ...
            animstate->offset = ImLerp(animstate->offset, cursor_screen_rect.Min.x - clickable.Min.x, gui->fixed_speed(20.f));
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(clickable.Min + ImVec2(animstate->offset, 12), ImVec2(clickable.Min.x + animstate->offset, clickable.Max.y - 11), GetColorU32(ImGuiCol_Text));
...
bool c_gui::text_field(std::string_view label, std::string_view icon, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return input_text_ex(label.data(), icon.data(), buf, (int)buf_size, size, flags, 0, 0);
}